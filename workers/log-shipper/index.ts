...
function buildKey(): string {
  ...
  // SEC: use crypto.getRandomValues() for 128-bit entropy instead of Math.random()
  // (~32-bit) to avoid silent key collision under burst traffic.
...
async function postAlert(env: TailWorkerEnv, payload: CloudflareTailEvent): Promise<void> {
  ...
  try {
    ...
    // Block localhost, RFC-1918, link-local, cloud metadata endpoints,
    // and IPv6 private ranges (ULA, link-local, IPv4-mapped) that were
    // previously absent from the blocklist.
