...
variable "stripe_webhook_dlq_bucket_name" {
  type        = string
  default     = "stripe-webhook-dlq"
  description = "R2 bucket that stores redacted Stripe webhook DLQ NDJSON records after retry exhaustion."
}
...
resource "cloudflare_r2_bucket" "stripe_webhook_dlq" {
  account_id = var.cloudflare_account_id
  name       = var.stripe_webhook_dlq_bucket_name
  location   = var.r2_default_location
}
...
output "stripe_webhook_dlq_bucket_name" {
  value       = cloudflare_r2_bucket.stripe_webhook_dlq.name
  description = "Name of the R2 bucket bound to STRIPE_WEBHOOK_DLQ_BUCKET in wrangler.jsonc."
}
...
output "r2_lifecycle_notice" {
  value       = <<-EOT
    ...
    Bucket: ${cloudflare_r2_bucket.stripe_webhook_dlq.name}
    - Lifecycle expiry:  ${var.r2_log_retention_days} days (match var.r2_log_retention_days)
    - WORM / object-lock: ${var.r2_worm_enabled ? "ENABLED" : "DISABLED"}
    - Replication:        ${var.r2_replication_enabled ? "ENABLED" : "DISABLED (enable in tfvars once GA)"}
...
locals {
  r2_retention_buckets = toset([
    cloudflare_r2_bucket.worker_logs.name,
    cloudflare_r2_bucket.stripe_webhook_dlq.name,
  ])
}

resource "null_resource" "r2_retention_lifecycle" {
  for_each = local.r2_retention_buckets
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      JSON
      npx --yes wrangler@4.85.0 r2 bucket lifecycle set ${each.value} --file "$RULE_FILE"
      rm -f "$RULE_FILE"
    EOT
