...
`lib/api-route-metadata.ts` and `lib/api-contract-schema.ts`. It now represents
the generate, draft-by-ID, publish, product, and owner action routes described
above, in addition to health, context, analytics summary, content list, draft
creation, runs, and service-account admin routes. The generator also brought
several other routes already present in the metadata registry into the checked
in artifact. Where the generated contract is envelope-level rather than a
field-by-field schema, this guide follows the route handlers and Zod parsers.
