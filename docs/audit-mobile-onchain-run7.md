# Mobile/On-Chain Security Audit -- Run 7 of 10
...
- `package.json` lists `next`, `react`, `react-dom`, `@opennextjs/cloudflare`,
  `@supabase/supabase-js`, `wrangler`.
...
- `app/` is Next.js App Router; `workers/` is the Cloudflare Worker entry;
  `supabase/` is SQL migrations.

**Conclusion for this run:** No mobile binary, no native client, no on-chain
code. Every A116--A135 control targets either a mobile binary/runtime or an EVM
smart-contract codebase. Per the user's skip rule, all items are N/A.
...
Evidence collected (Frida / class-dump / Slither / Echidna): **none** --
toolchains do not apply.

---

## Note

The user requested 10 sequential runs of the same ruleset. Because the ruleset
is mobile + on-chain only and the project is a server-rendered web app, every
run produces an identical N/A result. This is run 7 of 10; runs 1--10 are by
construction equivalent. If a web-app-equivalent ruleset is desired (OWASP ASVS
/ Top 10, Next.js middleware, Supabase RLS, CSP, CSRF, rate limiting, supply
chain, secrets), that would be a different scope.
