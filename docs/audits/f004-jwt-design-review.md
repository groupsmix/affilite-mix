...
| Stolen-token replay from another device | **Mitigated** | Optional `bnd` binding claim = hash of user-agent + role-aware IP fingerprint (super_admin `/32`, others `/24` to tolerate NAT). When a token *carries* `bnd` it is **always** verified regardless of the operator toggle; the flag only governs whether a token *missing* `bnd` is acceptable. `lib/jwt-binding.ts`, `lib/auth.ts`. |
...
1. **HS256 is symmetric.** The same secret signs and verifies. This is correct and simplest for a single-service admin surface. It would only need to move to an asymmetric algorithm (RS256/EdDSA) if a *separate* service ever had to verify admin tokens without holding the signing secret. No action needed today; noted so a future multi-service split revisits it.
