# Mobile / On-Chain Security Audit -- Runs 6, 8, 9 of 10
...
- **No** React Native, Capacitor, Cordova, Expo, Flutter, native iOS/Android,
  or smart-contract toolchain.
...
smart-contract codebase. Per the skip rule, all items are **N/A** for these
runs.
...
Applies identically to runs 6, 8, and 9.
...
All 20 controls evaluated as N/A. No mobile binary, no on-chain code detected.
Result is identical to all other runs by construction.
...
## Standing Note (applies to all 10 runs)
...
run produces an identical N/A result. Runs 1--10 are by construction equivalent.

If a web-app-equivalent ruleset is desired (OWASP ASVS / Top 10, Next.js
middleware, Supabase RLS, CSP, CSRF, rate limiting, supply chain, secrets),
that would be a different scope.
