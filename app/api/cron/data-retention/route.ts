...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error: clicksError } = await (query as any).unsafeNoSiteFilter();
  ...
  try {
    ...
    // Try the transactional RPC first. The current migration defines
    // purge_retention() with no arguments and returns per-table deletion counts.
    ...
    if (rpcError) {
      ...
      const { data: auditRows, error: fetchError } = await (query as any).unsafeNoSiteFilter();
      ...
      if (archiveSucceeded && auditRows && auditRows.length > 0) {
        ...
        const query = sb.from("audit_log").delete().in("id", ids);
        // eslint-disable-next-line no-restricted-syntax -- Audited: cron uses privileged client (no site header); gated by CRON_SECRET
        const { error: auditError } = await (query as any).unsafeNoSiteFilter();
      ...
      const result = rpcResult as { audit_log_deleted?: number } | null;
      results.audit_log = {
        success: true,
        archived: 0,
        deleted: result?.audit_log_deleted ?? 0,
      };
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
  ...
  try {
    ...
    const { error: stripeError } = await (query as any).unsafeNoSiteFilter();
