...
export async function POST(request: NextRequest) {
  ...
  if (isProd && rawMode === "permissive" && !permissivePosturedLogged) {
    ...
    logger.warn(
      "INTERNAL_HMAC_MIGRATION_MODE=permissive in production — legacy bearer fallback is enabled.",
  ...
  if (!hmacResult.valid) {
    ...
    if (migrationMode === "permissive") {
      ...
      logger.warn("Legacy bearer auth accepted on internal queue endpoint", {
  ...
  try {
    ...
    const rows: Array<{
      ...
      const row = {
        ...
        referrer: m.referrer ?? "",
        ...(m.click_id ? { click_id: m.click_id } : {}),
      };
      return row;
    });
