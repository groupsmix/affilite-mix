...
export async function POST(request: Request) {
  try {
    ...
    if (resendKey) {
      ...
      // A74: Bulkhead for Resend - cap concurrent/bursty email sends
      const bulkhead = await checkRateLimit("bulkhead:resend", {
        maxRequests: 10,
        windowMs: 1000, // 10 per second
        failPolicy: "open", // Don't block emails if rate limiter is down
      });

      if (!bulkhead.allowed) {
        // Return 200 anyway to prevent enumeration, but log the bulkhead hit
        logger.warn("Resend bulkhead hit in forgot-password", {
          siteId: site.id,
          retryAfterMs: bulkhead.retryAfterMs,
        });
      } else {
