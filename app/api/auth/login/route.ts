...
import { getAdminUserByEmail, updateAdminUser, incrementAdminLoginAttempts, incrementAdminTotpAttempts } from "@/lib/dal/admin-users";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (email.length > 255 || password.length > 72) {
      return apiError(400, "Invalid input length");
    }
    ...
    if (!authResult) {
      if (userRecord) {
        try {
          await incrementAdminLoginAttempts(userRecord.id);
        } catch (e: any) {
          // Ignore missing column errors if the migrations haven't run yet
    ...
    if (authResult.email) {
      ...
      if (user?.totp_enabled) {
        ...
        if (
          ...
          try {
            await incrementAdminTotpAttempts(user.id);
          } catch (e: any) {
            if (e.code !== "42883") {
              logger.error("Failed to increment TOTP attempts", { error: e });
            }
          }
    ...
    // A181: Trigger P0 alert if a break-glass account is activated
    if (authResult.email.startsWith("breakglass")) {
      logger.error("P0 ALERT: BREAK-GLASS ACCOUNT ACTIVATED", { email: authResult.email, ip });
      captureException(new Error(`P0 ALERT: Break-glass account ${authResult.email} activated from ${ip}`), { level: "fatal" });
    ...
    response.cookies.set(COOKIE_NAME, token, {
      ...
      sameSite: "strict",
      path: "/",
      maxAge: ADMIN_SESSION_MAX_AGE_SECONDS, // 4 hours (matches JWT expiry)
      domain: process.env.NODE_ENV === "production" ? request.nextUrl.hostname : undefined,
    });
