...
import { parseProductAffiliateUrlInput } from "@/lib/automation/schemas";
...
import {
  assertProductTarget,
  mapProductExecutorError,
  validateProductAffiliateDestination,
} from "@/lib/automation/executors/products";

const ACTION_TYPE = "products.update_affiliate_url" as const;
...
export const POST = withAutomation(
  ...
  async (request: NextRequest, { auth, requestId, params }) => {
    ...
    const parsed = parseProductAffiliateUrlInput({
    ...
    if (!parsed.ok) {
      return automationError(
        ...
        "Affiliate URL update failed validation",
        requestId,
        {
    ...
    const destinationError = validateProductAffiliateDestination(parsed.value.affiliate_url);
    if (destinationError) {
      return automationError("AUTOMATION_VALIDATION_ERROR", destinationError, requestId);
    }
