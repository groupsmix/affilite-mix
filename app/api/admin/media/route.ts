 ...
 */
export const GET = withAuthz("upload", "view", async (request, { siteId }) => {
  const { searchParams } = request.nextUrl;
  const limit = Math.min(200, Math.max(1, parseInt(searchParams.get("limit") ?? "50", 10) || 50));
  const offset = Math.max(0, parseInt(searchParams.get("offset") ?? "0", 10) || 0);

  const media = await listMedia({ siteId, limit, offset });
 ...
 */
export const DELETE = withAuthz("upload", "delete", async (request, { siteId }) => {
  const id = request.nextUrl.searchParams.get("id");
  if (!id) {
    return NextResponse.json({ error: "id is required" }, { status: 400 });
  }

  const media = await getMediaById(siteId, id);
  if (!media) {
