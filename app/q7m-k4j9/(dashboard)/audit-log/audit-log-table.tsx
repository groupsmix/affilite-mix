 ...
 * link to `/q7m-k4j9/users?q=<email>` — the admin-users page supports an
 * email/name `q` search and will land the reviewer on the matched row.
 * There is no dedicated `/q7m-k4j9/users/[id]` detail route in the app today
...
function ActorCell({ row }: { row: AuditLogTableRow }) {
  ...
  if (row.actor && row.actor_user_id) {
    return (
      <Link
        href={`/q7m-k4j9/users?q=${encodeURIComponent(row.actor)}`}
        title={actor}
