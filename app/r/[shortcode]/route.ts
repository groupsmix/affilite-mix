...
export async function GET(
  ...
  try {
    ...
    try {
      ...
      // SEC-03: Prevent redirect loops back to our own tracking paths
      const reqHost = request.headers.get("host") || request.nextUrl.host;
      if (urlObj.pathname.startsWith("/r/") && urlObj.host === reqHost) {
        logger.error("[r/shortcode] rejected redirect: redirect loop detected", {
          siteId,
          shortcode,
          destination: destinationUrl,
        });
        return apiError(400, "Redirect loop detected");
      }
    } catch {
