...
const STANDARD_PUBLIC_PORTS = new Set(["80", "443"]);

function getExplicitRequestPort(request: NextRequest): string | null {
  const host = request.headers.get("host") ?? "";
  const bracketedIpv6Match = host.match(/^\[[^\]]+\]:(\d+)$/);
  if (bracketedIpv6Match) return bracketedIpv6Match[1];
  ...
  return request.nextUrl.port || null;
}

function isLocalhostHostname(hostname: string): boolean {
  const hostWithoutPort = hostname.includes(":") ? hostname.split(":")[0] : hostname;
  return hostWithoutPort === "localhost" || hostWithoutPort.endsWith(".localhost");
}
...
async function innerMiddleware(request: NextRequest) {
  ...
  // A206/A209/A213: Cloudflare-proxied zones may accept traffic on
  // alternate HTTP ports (for example 8080). Keep application access
  // constrained to the standard public web ports; Terraform also blocks
  // these at WAF level, but this app-side guard protects preview/staging
  // deployments before the zone rule is applied.
  const explicitPort = getExplicitRequestPort(request);
  if (
    process.env.NODE_ENV === "production" &&
    explicitPort &&
    !STANDARD_PUBLIC_PORTS.has(explicitPort) &&
