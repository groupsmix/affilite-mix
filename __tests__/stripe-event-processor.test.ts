...
function makeStripe(): StripeStub {
  return {
    ...
    invoices: {
      retrieve: vi.fn(async () => ({ subscription: "sub_from_invoice" })),
    },
    charges: {
      retrieve: vi.fn(async (id: string) => ({ id, invoice: "in_123" })),
    },
  };
}
...
describe("processStripeEvent — atomic idempotency (LIVE-10 / F-024)", () => {
  ...
  it("customer.subscription.updated mirrors Stripe status and tier changes", async () => {
  ...
  it("invoice.payment_failed marks the membership past_due", async () => {
    rpcMock.mockResolvedValueOnce({
      data: { duplicate: false, membership_id: "mem_failed" },
      error: null,
    });
    ...
    const event = {
      ...
      data: { object: { subscription: "sub_failed" } },
    } as any;
  ...
  it("charge.dispute.closed lost cancels the membership", async () => {
    rpcMock.mockResolvedValueOnce({
      data: { duplicate: false, membership_id: "mem_dispute" },
      error: null,
    });
    ...
    expect(rpcMock).toHaveBeenCalledWith("apply_stripe_membership_event", {
      p_stripe_event_id: "evt_dispute_lost",
      p_event_type: "charge.dispute.closed",
      p_event_data: {
        op: "cancel_membership",
        stripe_subscription_id: "sub_from_invoice",
      },
    });
  });

  it("customer.subscription.resumed restores the mirrored Stripe status", async () => {
    rpcMock.mockResolvedValueOnce({
      data: { duplicate: false, membership_id: "mem_resumed" },
      error: null,
    });
    ...
    const event = {
      ...
      data: { object: { id: "sub_resumed", status: "active" } },
    } as any;
    ...
    expect(result.duplicate).toBe(false);
    expect(rpcMock).toHaveBeenCalledWith("apply_stripe_membership_event", {
      p_stripe_event_id: "evt_resumed",
      p_event_type: "customer.subscription.resumed",
      p_event_data: {
        op: "update_status",
