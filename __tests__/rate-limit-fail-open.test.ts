...
describe("F-3 KV unavailability", () => {
  ...
  it("does not leak stale in-memory state across requests (M4 mutation guard)", async () => {
    vi.stubEnv("NODE_ENV", "development");
    vi.useFakeTimers();
    const t0 = Date.UTC(2025, 0, 1, 0, 0, 0);
    vi.setSystemTime(t0);
    ...
    await checkRateLimit("ip:stale-test", CONFIG);
    await checkRateLimit("ip:stale-test", CONFIG);
    
    // Should be blocked
    const blocked = await checkRateLimit("ip:stale-test", CONFIG);
    expect(blocked.allowed).toBe(false);

    // Advance time past the windowMs
    vi.setSystemTime(t0 + CONFIG.windowMs + 1);

    // Should be allowed again because stale state is cleaned up
    const allowedAgain = await checkRateLimit("ip:stale-test", CONFIG);
    expect(allowedAgain.allowed).toBe(true);
  });

  it("prevents integer overflow on huge maxRequests (M5 mutation guard)", async () => {
    vi.stubEnv("NODE_ENV", "development");
    const { checkRateLimit } = await loadModule();

    const hugeConfig = { maxRequests: Number.MAX_SAFE_INTEGER, windowMs: 60_000 };
    const result = await checkRateLimit("ip:huge-test", hugeConfig);
