...
describe("cron authentication", () => {
  it("rejects missing authorization header", () => {
    // `let` (not `const`) preserves the broader `string | null` type so the
    // comparison below is a valid string-vs-string check
    // (CodeQL js/comparison-between-incompatible-types narrows const + literal).
    let authHeader: string | null = null;
    // Reassign once so CodeQL doesn't constant-fold the value back to null.
    authHeader = process.env.UNSET_ENV_VAR ?? null;
