...
describe("constructStripeEvent", () => {
  ...
  it("rejects signatures using the legacy v0= format (mutation guard)", async () => {
    const payload = JSON.stringify({
      id: "evt_v0",
      type: "checkout.session.completed",
      data: { object: {} },
    ...
    const header = await sign(payload, SECRET, Math.floor(Date.now() / 1000));
    // The signature itself is valid, but it's passed as v0= instead of v1=
    const v0Header = header.replace("v1=", "v0=");
    
    await expect(constructStripeEvent(payload, v0Header, SECRET)).rejects.toThrow(
      StripeSignatureError
    );
  });

  it("prewarm key mismatch correctly generates a new key instead of using cached one", async () => {
    const { prewarmStripeWebhookKey } = await import("@/lib/stripe-webhook");
    const payload = JSON.stringify({
      id: "evt_prewarm",
      type: "test",
      data: { object: {} },
    ...
    // Prewarm with wrong secret
    await prewarmStripeWebhookKey(wrongSecret);
    
    // Generate valid header using correct secret
    const header = await sign(payload, correctSecret, Math.floor(Date.now() / 1000));
    
    // Attempt to verify using correct secret.
    // If the prewarmed key for the wrong secret is incorrectly used, this will fail.
    const event = await constructStripeEvent(payload, header, correctSecret);
    expect(event.id).toBe("evt_prewarm");
  });
});
