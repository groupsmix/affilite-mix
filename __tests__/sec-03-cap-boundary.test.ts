...
describe("SEC-03 allowlist cap (Task 5.4)", () => {
  it("the SEC-03 cap is 40 (R10.5)", () => {
    expect(SEC_03_CAP).toBe(40);
  });
...
describe("SEC-03 boundary behavior (Task 5.4)", () => {
  it("passes when the allowlist has fewer than 40 entries (R10.6)", () => {
    expect(sec03Passes(0)).toBe(true);
    expect(sec03Passes(38)).toBe(true);
    expect(sec03Passes(39)).toBe(true);
  });

  it("passes at exactly 40 entries — the cap is inclusive (R10.6)", () => {
    expect(sec03Passes(40)).toBe(true);
  });

  it("fails when the allowlist exceeds 40 entries (R10.7)", () => {
    expect(sec03Passes(41)).toBe(false);
