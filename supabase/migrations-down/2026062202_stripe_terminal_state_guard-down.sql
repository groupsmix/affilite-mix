-- Down migration for 2026062202_stripe_terminal_state_guard
--
-- Restores the previous (2026052905) definition of
-- apply_stripe_membership_event, removing the F1 terminal-state guard so that
-- renew_membership / update_status once again unconditionally set status =
-- 'active'. WARNING: reverting re-opens the F1 entitlement-restoration residual
-- (a refunded/charged-back membership can be flipped back to 'active' by the
-- next invoice.paid / customer.subscription.updated webhook).
