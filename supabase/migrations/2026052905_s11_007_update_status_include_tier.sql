-- ============================================================
-- Migration 2026052905: S11-007 — include tier in update_status branch
--
-- When customer.subscription.updated fires (e.g. plan upgrade from
-- 'insider' to 'pro'), the stripe-event-processor resolves the new
-- tier from the Stripe price ID and includes `tier: newTier` in the
-- payload. However the SQL function's update_status branch only set
-- status and updated_at — it ignored the tier key.
--
-- Fix: add COALESCE(NULLIF(p_event_data ->> 'tier', ''), tier) so
-- the tier is updated when present, and preserved when absent/empty.
-- ============================================================
