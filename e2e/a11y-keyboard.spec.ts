...
test.describe("Focus indicators are visible", () => {
  test("primary CTAs render a focus ring/outline when focused", async ({ page }) => {
    ...
    expect(await candidate.count(), "no interactive elements found on this site's homepage").toBeGreaterThan(0);
...
test.describe("Form error a11y", () => {
  test("admin login surfaces credential failures via role=alert", async ({ page }) => {
    ...
    expect(res?.status() ?? 500, "admin login route not reachable on this build").toBeLessThan(500);
    await page.waitForLoadState("networkidle");
    ...
    expect(await email.count(), "admin login form not present on this tenant").toBeGreaterThan(0);
    expect(await password.count(), "admin login form not present on this tenant").toBeGreaterThan(0);
    ...
    expect(await submit.isDisabled(), "submit button disabled (Turnstile required)").toBe(false);
    await submit.click();
