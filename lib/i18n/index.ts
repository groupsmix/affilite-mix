...
const EN_MESSAGES: Record<string, string> = {
  ...
  "newsletter.confirm_plain_thanks": "Thanks for subscribing to {siteName}!",
  "newsletter.confirm_plain_link_prompt": "Please confirm your email by visiting the link below:",
...
const AR_MESSAGES: Record<string, string> = {
  ...
  "newsletter.confirm_plain_thanks": "شكراً لاشتراكك في {siteName}!",
  "newsletter.confirm_plain_link_prompt": "يرجى تأكيد بريدك الإلكتروني بزيارة الرابط أدناه:",
...
/**
 * E2-013: Format a localized message with interpolation and basic pluralization.
 *
 * Supports:
 *   - Simple interpolation: `{name}` → replaces with params.name
 *   - Plural rules: `{count, plural, one {# item} other {# items}}`
 *     Uses Intl.PluralRules for CLDR-compliant plural categories.
 *   - Number formatting: `{amount, number}` → locale-aware number format
 *
 * Falls back to `t()` when no params are provided.
 */
export function tFormat(
  ...
  message = message.replace(
    ...
    (_match, paramName: string, cases: string) => {
      ...
      while ((caseMatch = caseRegex.exec(cases)) !== null) {
        caseMap.set(caseMatch[1], caseMatch[2]);
      }

      const template = caseMap.get(category) ?? caseMap.get("other") ?? String(num);
      return template.replace(/#/g, String(num));
    },
  );

  // Number formatting: {amount, number}
  message = message.replace(/\{(\w+),\s*number\}/g, (_match, paramName: string) => {
    const value = params[paramName];
    if (value === undefined) return _match;
    const num = typeof value === "number" ? value : Number(value);
