...
/**
 * Screen user-supplied input (topic, keywords) for prohibited content and prompt injections.
...
export function moderateInput(topic: string, keywords: string[] = []): ModerationResult {
  ...
  if (JAILBREAK_PATTERNS.some((pattern) => pattern.test(combined))) {
    return { passed: false, reason: "Input contains suspected prompt injection or jailbreak patterns" };
  }
