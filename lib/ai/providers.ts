...
const BREAKER_RESET_SECONDS = 300; // 5 minutes
...
async function isCircuitOpen(providerName: string): Promise<boolean> {
  const kv = getRateLimitKv();
  if (!kv) return false;

  const key = `ai-breaker:${providerName.toLowerCase().replace(/\s+/g, "-")}`;
  try {
    const failures = parseInt((await kv.get(key)) || "0", 10);
    return failures >= BREAKER_THRESHOLD;
  } catch {
    return false; // Fail open on KV error
  }
}

async function recordFailure(providerName: string): Promise<void> {
  const kv = getRateLimitKv();
  if (!kv) return;
  ...
  try {
    ...
    await kv.put(key, (current + 1).toString(), { expirationTtl: BREAKER_RESET_SECONDS });
  } catch {
    // Ignore KV errors in breaker
  }
}

async function recordSuccess(providerName: string): Promise<void> {
  const kv = getRateLimitKv();
  if (!kv) return;
...
export async function generateWithFallback(
  ...
  for (const provider of ALL_PROVIDERS) {
    ...
    if (await isCircuitOpen(provider.name)) {
      errors.push(`${provider.name}: circuit open (too many recent failures)`);
      continue;
    }
    ...
    try {
      ...
      void recordFailure(provider.name);
