...
export function sanitizePrompt(input: string, options: SanitizePromptOptions = {}): string {
  ...
  // 0c. Redact PII (A104) before it hits external models.
  // Mask SSNs, basic credit cards, and US phone numbers.
  out = out.replace(/\b\d{3}[-.\s]?\d{2}[-.\s]?\d{4}\b/g, "[REDACTED_SSN]");
  out = out.replace(/\b(?:\d[ -]*?){13,16}\b/g, "[REDACTED_CARD]");
  out = out.replace(/\b(?:\+?1[-.\s]?)?(?:\(\d{3}\)|\d{3})[-.\s]?\d{3}[-.\s]?\d{4}\b/g, "[REDACTED_PHONE]");
