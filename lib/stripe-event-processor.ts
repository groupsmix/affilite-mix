...
function getDisputeChargeId(dispute: Stripe.Dispute): string | undefined {
...
async function getSubscriptionIdForCharge(
  stripe: Stripe,
  ...
  try {
    ...
    logger.error(`Failed to retrieve invoice from Stripe in ${context}`, {
      invoiceId,
      error: err,
    });
    return undefined;
...
async function buildStripeEventPayload(
  ...
  switch (event.type) {
    ...
    case "charge.refunded": {
      ...
      const subscriptionId = await getSubscriptionIdForCharge(stripe, charge, "charge.refunded");
      if (!subscriptionId) return { op: "noop" };
    ...
    case "charge.dispute.closed": {
      ...
      const subscriptionId = await getSubscriptionIdForCharge(stripe, charge, "dispute handler");
      if (!subscriptionId) return { op: "noop" };
      ...
      if (event.type === "charge.dispute.closed") {
        ...
        try {
          ...
          logger.error("Failed to retrieve subscription from Stripe in dispute closed handler", {
            subscriptionId,
            error: err,
          });
          return { op: "noop" };
