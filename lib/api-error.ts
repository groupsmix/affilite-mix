 ...
 *
 * A91 Fix: In production, 500 errors never return the raw error message or
 * stack traces in `details` to prevent information disclosure.
 */
export function apiError(
  ...
  const errorCode = code ?? defaultCodeForStatus(status);
  ...
  if (status >= 500 && isProd) {
    ...
    // Never leak raw error objects in production
  ...
  if (safeDetails !== undefined) {
    // A91: Ensure details doesn't contain Error objects which serialize poorly
    // and might leak sensitive internal paths/stacks.
    if (safeDetails instanceof Error) {
      body.details = isProd ? undefined : { message: safeDetails.message, stack: safeDetails.stack };
    } else {
