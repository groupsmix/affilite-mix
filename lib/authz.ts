...
export function withAuthz(
  ...
  return async (request: NextRequest) => {
    ...
    if (session.role === "break_glass") {
      void recordAuditEvent({
        site_id: dbSiteId,
        actor: session.email ?? session.userId ?? "break_glass",
        actor_user_id: session.userId,
        action: `break_glass_access`,
        entity_type: feature,
        entity_id: "all",
        details: { method: request.method, url: request.url, action },
      });
    }
  ...
  return async (request: NextRequest, { params }: { params: Promise<Record<string, string>> }) => {
    ...
    if (session.role === "break_glass") {
      void recordAuditEvent({
        ...
        entity_type: feature,
        entity_id: "dynamic",
        details: { method: request.method, url: request.url, action, params: resolvedParams },
      });
    }
...
export async function authorizeResource(
  ...
  if (opts.session?.role === "break_glass") {
    void recordAuditEvent({
      site_id: realSiteId,
      actor: opts.session.email ?? opts.session.userId ?? "break_glass",
      actor_user_id: opts.session.userId,
      action: `break_glass_resource_access`,
      entity_type: opts.resourceType,
      entity_id: opts.resourceId,
      details: { feature: opts.feature, action: opts.action },
    });
  }
