...
export async function getAdminUserByEmail(
  ...
  const { data, error } = await sb
    .from(TABLE)
    .select("id, email, password_hash, role, totp_enabled, totp_secret, totp_failed_attempts, totp_locked_until, login_failed_attempts, login_locked_until, name, is_active, created_at")
...
/** Atomically increment login failed attempts */
export async function incrementAdminLoginAttempts(
  id: string,
  getClient: DalClientGetter = defaultDalClientGetter,
): Promise<void> {
  const sb = await getClient();
  const { error } = await sb.rpc("increment_admin_login_attempts", { p_user_id: id });
  if (error) throw error;
}

/** Atomically increment totp failed attempts */
export async function incrementAdminTotpAttempts(
  id: string,
  getClient: DalClientGetter = defaultDalClientGetter,
): Promise<void> {
  const sb = await getClient();
  const { error } = await sb.rpc("increment_admin_totp_attempts", { p_user_id: id });
  if (error) throw error;
}
