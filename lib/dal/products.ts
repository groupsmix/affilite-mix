 ...
 * ConflictError is thrown so the caller can inform the user about the stale
 * write. The `version` column is always bumped server-side via a raw increment.
 */
...
export async function updateProduct(
  ...
  // Build the query — always filter by site + id
  let query = sb
    .from(TABLE)
    .update({ ...input, version: expectedVersion !== undefined ? expectedVersion + 1 : 1 })
    .eq("site_id", siteId)
    .eq("id", id);
  ...
  // match zero rows and PostgREST returns PGRST116 (no rows found).
