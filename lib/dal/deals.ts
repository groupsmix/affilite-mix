...
import { defaultDalClientGetter, assertSiteIdForPrivilegedClient, type DalClientGetter } from "./dal-client";
...
export async function listActiveDeals(
  ...
  assertSiteIdForPrivilegedClient(sb, siteId);
...
export async function getDealById(
  ...
  if (error) throw error;
  const row = rowOrNull<DealRow>(data);
  if (row) {
    assertSiteIdForPrivilegedClient(sb, row.site_id);
  }
  return row;
}
...
export async function createDeal(
  ...
  const sb = await getClient();
  assertSiteIdForPrivilegedClient(sb, input.site_id);
