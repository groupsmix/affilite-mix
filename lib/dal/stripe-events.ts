...
export async function applyStripeEventAtomic(
  ...
  // A-97 Guard: Require site_id for create_membership op
  if (payload.op === "create_membership") {
    assertSiteIdForPrivilegedClient(sb, payload.site_id);
  }
...
export async function recordStripeEvent(
  ...
  const query = sb.from(TABLE).insert({
    stripe_event_id: stripeEventId,
    event_type: eventType,
  });

  // F-API-01: stripe_events is a cross-tenant table (no site_id column).
  // Explicitly opt out of the site_id filter enforcement.
  const { error } = await (query as any).unsafeNoSiteFilter();
...
export async function getRecentStripeEventIds(
  ...
  // F-API-01: stripe_events is a cross-tenant table (no site_id column).
  const { data, error } = await (query as any).unsafeNoSiteFilter();
